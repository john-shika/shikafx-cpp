...
static const char BASE64_CHARACTERS_SIZE = 64;
...
data_t* base64_decode_func(const base64_data_t* base64_data);
data_t* base64_data_to_ascii_cvt_func(const base64_data_t* data, bool padding);
base64_data_t* ascii_to_base64_data_cvt_func(const char* val, size_t size);
data_t* copy_byte_stack_to_data_func(const byte_t* buff, size_t size);
